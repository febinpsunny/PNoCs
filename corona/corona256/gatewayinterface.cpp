...
void TGatewayInterface::Optical_rxProcess() // Clock Edge
{
	if (reset.read())
    {
			  ...
			  previous_transfer_photonic = -10;
			  previous_transfer_srcid  = -6;
	  
	}
  else
    {
			  TFlit received_flit_temp = mwmr_data_rx.read();
			  if(received_flit_temp.dst_hub_id == local_id_new && received_flit_temp.flit_type == FLIT_TYPE_HEAD)
			  {
			  if(previous_transfer_photonic != received_flit_temp.timestamp && !buffer_receive.IsFull() )
			  {
					  if(TGlobalParams::verbose_mode > VERBOSE_OFF)
					  {
						  cout<< sc_time_stamp().to_double()/1000 << " gwi Source_Id --> " <<  received_flit_temp.src_hub_id <<" gwi Destination_Id --> " <<  received_flit_temp.dst_hub_id << endl;	   
					  }
					  previous_transfer_photonic = received_flit_temp.timestamp;
					  previous_transfer_srcid = received_flit_temp.src_id;
					  int src_cluster = received_flit_temp.src_hub_id/8;
					  int dst_cluster = received_flit_temp.dst_hub_id/8;
					  ...
					  stats.receivedFlit((sc_time_stamp().to_double()/1000+ optical_delay), stat_flit);
				  ...
				  {
					  if(previous_transfer_srcid != received_flit_temp.src_id && !buffer_receive.IsFull() )
					  {
					  if(TGlobalParams::verbose_mode > VERBOSE_OFF)
					  {
						  cout<< sc_time_stamp().to_double()/1000 << " gwi Source_Id2 --> " <<  received_flit_temp.src_hub_id <<" gwi Destination_Id --> " <<  received_flit_temp.dst_hub_id <<" Channel number : " << endl;	   
					  }
					  previous_transfer_photonic = received_flit_temp.timestamp;
					  previous_transfer_srcid = received_flit_temp.src_id;
					  int src_cluster = received_flit_temp.src_hub_id/8;
	  ...
	  if ( (req_rx.read()==1-current_level_rx) && !buffer_local.IsFull() )
	    {
	      ...
	      if(TGlobalParams::verbose_mode > VERBOSE_OFF) //&& received_flit.src_id ==0)
		{
...
void TGatewayInterface::mwmr_tx_data() // sensitive - Clock
{
  ...
  else if(!reset.read() ) 
  {
		  if(!buffer_local.IsEmpty() && req_rx_arbiter_ch.read()==1-arbiter_current_level_rx_ch)
		  {
			  ...
			  mwmr_data_tx[mwsr_channel_send.read()].write(tx_flit);
			  ...
			  if(tx_flit.flit_type == FLIT_TYPE_HEAD) 
			  {
				  ...
				  if(TGlobalParams::verbose_mode > VERBOSE_OFF )
				  {
					  cout<< sc_time_stamp().to_double()/1000  << "Source_Id --> " <<  tx_flit.src_id <<" Destination_Id --> " <<  tx_flit.dst_id << "channel send: " << mwsr_channel_send.read() << endl;
				  }
  ...
  if(!reset.read() && ack_tx_arbiter_hf.read() == arbiter_current_level_tx_req && !buffer_local.IsEmpty())
  {
	  if(previous_transfer != buffer_local.Front().timestamp)
	  {
       ...
       // sending head flit to arbiter 
		  ...
		  prev_srcid = flit.src_id;
		  prev_dstid = flit.dst_id;
		   if(TGlobalParams::verbose_mode > VERBOSE_OFF)
	   {
         cout<< sc_time_stamp().to_double()/1000  << "Previous transfer:" << previous_transfer << "From Request hub Source_Id --> " <<  flit.src_hub_id <<" Head_Destination_hub_Id --> " <<  flit.dst_hub_id <<" local id " << local_id_new << endl; 	   
	   }
   ...
   {
	   if(prev_srcid != buffer_local.Front().src_id || prev_dstid != buffer_local.Front().dst_id)
	  {
       TFlit flit = buffer_local.Front();
       // sending head flit to arbiter 
		  req_tx_arbiter_flit.write(flit);
