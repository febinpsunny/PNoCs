...
{
	//intialization of node on a photonic waveguide
	circularbuffer::node* cbuffer = new node[size];
	for(int i=0; i< clusters ; i++)
	{ 
		countlocal[i]=0; // Resetting local count to zero
	}
	...
	int basicslot = 2 * total_priority; // Determination of basic slots based on the current priority
	count = std::abs((basicslot-1) -(tokensync % basicslot)); // intializing injection to tokens based the token synchronization value 	
	// This loop intialises basic token pattern which is used for actual injection of tokens
	int temp = *(current_priority); 
	int j =0;
	for(int i=0; i< (2 * total_priority) ; i++)
	{ 
	...
	// This loop intialises the circular buffer based on the current priority of clusters.
	for(int i=0; i<size; i++)
	{
		...
		else
		{
			...
			if(basicslot == 0)
			{
				basicslot = 2*total_priority;
			}
		}	
		cbuffer[i].destinationid = -1;	
		cbuffer[i].slotid = i;
		//cout << "Current slot number:" << i << " Current token :" << cbuffer[i].tokentype<< " Current flag :" << cbuffer[i].flag << endl;
		// This part of the loop form a circular buffer, further two head pointers are assigned for first and second pass of waveguide respectively. 
		if(i != size-1 && i != 0)
		{
...
void circularbuffer::change_token_priority(int clusters, int tokensync, int* current_priority)
{
	...
	int total_priority =0;
	int j =0;
	//Total priority of all the clusters is calculated inorder to calculate perodicity of the current tokens.
	for(int c=0; c<clusters; c++)
	{
		total_priority = total_priority + *(current_priority +c);
	}
	count = std::abs(((2 * total_priority)-1) -(tokensync % (2 * total_priority)));
	int temp = *(current_priority);  	
	for(int i=0; i< (2 * total_priority) ; i++)
	{ 
...
void circularbuffer::rotate(int clusters, int* current_priority)    // Function that rotates circular buffer and injects updated token into waveguide.
{
