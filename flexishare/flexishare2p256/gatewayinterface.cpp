  ...
  else
    {
		  for(int i=0; i< TGlobalParams::mwmr_waveguides; i++)
		  {
			  ...
			  if(previous_transfer_photonic[i] != received_flit_temp.timestamp && !buffer_receive.IsFull() )
			  {
					  ...
					  {
						  cout<< sc_time_stamp().to_double()/1000 << " gwi Source_Id1 --> " <<  received_flit_temp.src_hub_id <<" gwi Destination_Id --> " <<  received_flit_temp.dst_hub_id <<" Channel number : " << i << endl;	   
					  }
				  ...
				  else
				  {
					  ...
					  if(TGlobalParams::verbose_mode > VERBOSE_OFF)
					  {
						  cout<< sc_time_stamp().to_double()/1000 << " gwi Source_Id2 --> " <<  received_flit_temp.src_hub_id <<" gwi Destination_Id --> " <<  received_flit_temp.dst_hub_id <<" Channel number : " << i << endl;	   
					  }
					  previous_transfer_photonic[i] = received_flit_temp.timestamp;
					  previous_transfer_srcid[i] = received_flit_temp.src_id;
					  int src_cluster = received_flit_temp.src_hub_id/8;
    ...
    { // Clear outputs and indexes of receiving protocol
  ...
  else
     {
	  ...
	  if ( (req_rx.read()==1-current_level_rx) && !buffer_local.IsFull() )
		...
		{
		  cout << sc_time_stamp().to_double()/1000 << ": GWI["<< local_id_new <<"], Input, Received flit: " << received_flit << " src Hub id: " << received_flit.src_hub_id <<" local id " << local_id_new << " dst Hub id: "<< received_flit.dst_hub_id <<endl;
		}
		assert(received_flit.src_hub_id != received_flit.dst_hub_id);
...
void TGatewayInterface::mwmr_tx_data() // sensitive - Clock
{
  ...
  else if(!reset.read() ) 
  {
		  if(!buffer_local.IsEmpty() && req_rx_arbiter_ch.read()==1-arbiter_current_level_rx_ch)
		  {
			  ...
			  mwmr_data_tx[mwmr_channel_no.read()].write(tx_flit);
			  ...
			  if(tx_flit.flit_type == FLIT_TYPE_HEAD) 
			  {
				  ...
				  if(TGlobalParams::verbose_mode > VERBOSE_OFF )
				  {
					  cout<< sc_time_stamp().to_double()/1000  << "Source_hub_Id --> " <<  tx_flit.src_hub_id << " local id " << local_id_new <<" Destination_hub_Id --> " <<  tx_flit.dst_hub_id << endl;
				  }
