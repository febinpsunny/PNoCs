...
SC_MODULE(TGatewayInterface)
{
  ...
  sc_out<TFlit>                  mwmr_data_tx[DEFAULT_MWMR_WAVEGUIDES];
  sc_in<TFlit>                   mwmr_data_rx[DEFAULT_MWMR_WAVEGUIDES];
  ...
  SC_HAS_PROCESS(TGatewayInterface);
  TGatewayInterface(sc_module_name name, TStats &_stats):
  stats(_stats)
  {
  ...
  double 			 previous_transfer_photonic[DEFAULT_MWMR_WAVEGUIDES];
  int 				 previous_transfer_srcid[DEFAULT_MWMR_WAVEGUIDES];
