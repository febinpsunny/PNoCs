...
SC_MODULE(TGITile)
{
  ...
  sc_out<TFlit>       flit_tx[MAX_STATIC_DIM][MAX_STATIC_DIM];   // The output channels
  ...
  SC_CTOR(TGITile)
  {
    ...
    for(int i=0; i<TGlobalParams::mesh_dim_x; i++)
    {
      ...
      {
      if(gwi->local_xid != i && gwi->local_yid != j)
      { 
		  gwi->mwmr_data_tx[i][j](flit_tx[i][j]);
	  }else
	  { 
		  gwi->mwmr_data_rx[gwi->local_xid][gwi->local_yid](flit_rx);
	  }  
	  }
    }
