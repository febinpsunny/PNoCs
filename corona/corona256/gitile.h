...
SC_MODULE(TGITile)
{
  ...
  sc_out<TFlit>       flit_tx[DEFAULT_MWMR_WAVEGUIDES];   // The output channels
  ...
  sc_in<int>             mwsr_channel_send; // MWSR channel ack from arbiter to send data
  ...
  TGITile(sc_module_name name, char* arg_vet)
  ...
  {  
	...
	gwi->mwsr_channel_send(mwsr_channel_send);
	...
	gwi->mwmr_data_rx(flit_rx); // Extension of input channels to tile
    ...
    pe[i] = new TProcessingElement(processing_element, PIR_STR);
    pe[i]->clock(clock);
    pe[i]->reset(reset);
