		  ...
		  for(int i=0; i<TGlobalParams::mesh_dim_x; i++)
		  {
			  for(int j=0; j<TGlobalParams::mesh_dim_y; j++)
			  {
				  ...
				  src_coord.y = newid2Coord(received_flit.src_hub_id).y;
				    ...
				    {
						cout<< sc_time_stamp().to_double()/1000  << " From Arbiter Source_Id --> " <<  received_flit.src_id <<" Head_Destination_Id --> " <<  received_flit.dst_id << endl;
						}
						}
						else
						{
  ...
  else
  {
	  if(!reset.read())
	  {
		  for(int i=0; i<TGlobalParams::mesh_dim_x; i++)
		  {
			  ...
			  {
				  if (node_packet_status[i][j] == 1 )
				  {
					  TCoord current_coord;
					  current_coord.x = i;
					  current_coord.y = j;
					  int index = coord2newId(current_coord);
					  int cfp; // channel number
					  bool kfp = cb->assign_first_pass(index, &cfp);
					  if(kfp == 1 && cfp!=-1)                     
						{
							if(ack_rx_arbiter_ch[i][j].read() == gwi_current_level_tx_ch[i][j])
							{
								mwmr_channel_no[i][j].write(cfp); // Forwarding channel number to GWI to write data
								node_packet_status[i][j] = 0;
								if(TGlobalParams::verbose_mode > VERBOSE_OFF)
								{
									cout<< sc_time_stamp().to_double()/1000  << "cooridateup [" <<  i <<"] [" << j <<"] "<<"index:"<< index << " channel number: "<< cfp << " channel availability: "<< kfp << endl;
								}
								gwi_current_level_tx_ch[i][j] = 1-gwi_current_level_tx_ch[i][j];
								req_rx_arbiter_ch[i][j].write(gwi_current_level_tx_ch[i][j]); 
							}
						}
					}
				}
				  ...
				  int k;
				   ...
				   for(int k=0; k<8; k++)
				   {
					...
					if(!node_index.empty() && node_index.front() != -1)
					{
						...
						node_index.clear();
						int c;
						bool k1 = cb->assign_second_pass(winner, &c); // sending indexes of winner nodes to arbitration waveguide for sanction of token
