...
#define DEFAULT_SIMULATION_TIME                  35000
...
struct TFlit
{
  ...
  {
    return (flit.src_id==src_id && flit.dst_id==dst_id && flit.src_hub_id==src_hub_id && flit.dst_hub_id==dst_hub_id && flit.nearest_gateway_id==nearest_gateway_id
...
struct TPacket
{
  ...
  TPacket() {;}
  TPacket(const int s, const int d, const int sh, const int dh, const double ts, const int sz) {
    make(s, d, sh, dh, ts, sz);
  }

  void make(const int s, const int d, const int sh, const int dh, const double ts, const int sz) {
    src_id = s; dst_id = d; src_hub_id=sh; dst_hub_id=dh; timestamp = ts; size = sz; flit_left = sz;
  }
};
