  ...
  else
    {
		  for(int i=0; i< TGlobalParams::mwmr_waveguides; i++)
		  {
			  for(int j=0; j<4; j++)
			 {
			  ...
			  if(received_flit_temp.dst_hub_id == local_id_new && received_flit_temp.flit_type == FLIT_TYPE_HEAD && received_flit_temp.dst_id != received_flit_temp.src_id )
			  {
			  if(previous_transfer_photonic[i][j] != received_flit_temp.timestamp && !buffer_receive.IsFull() )
			  {
					  if(TGlobalParams::verbose_mode > VERBOSE_OFF)
					  {
						  cout<< sc_time_stamp().to_double()/1000 << " gwi Source_Id --> " <<  received_flit_temp.src_hub_id <<" gwi Destination_Id --> " <<  received_flit_temp.dst_hub_id <<" Channel number : " << i <<" Sub Channel Number: "<< j << endl;	   
					  }
					  previous_transfer_photonic[i][j] = received_flit_temp.timestamp;
					  previous_transfer_srcid[i][j] = received_flit_temp.src_id;
				  ...
				  {
					  if( previous_transfer_srcid[i][j] != received_flit_temp.src_id && !buffer_receive.IsFull() )
					  {
					  if(TGlobalParams::verbose_mode > VERBOSE_OFF)
					  {
						  cout<< sc_time_stamp().to_double()/1000 << " gwi Source_Id --> " <<  received_flit_temp.src_hub_id <<" gwi Destination_Id --> " <<  received_flit_temp.dst_hub_id <<" Channel number : " << i <<" Sub Channel Number: "<< j << endl;	   
  ...
  else if(!reset.read() ) 
  {
		  if(!buffer_local.IsEmpty() && req_rx_arbiter_ch.read()==1-arbiter_current_level_rx_ch)
		  {
			  ...
			  if(tx_flit.flit_type == FLIT_TYPE_HEAD) 
			  {
				  ...
				  if(TGlobalParams::verbose_mode > VERBOSE_OFF )
				  {
					  cout<< sc_time_stamp().to_double()/1000  << "Source_Id --> " <<  tx_flit.src_hub_id <<" Destination_Id --> " <<  tx_flit.dst_hub_id << " channel number : "<< mwmr_channel_no.read() << "Sub channel number : "<< count << endl;
				  }
