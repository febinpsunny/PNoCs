...
#define TRAFFIC_RANDOM_BROADCAST    8
...
#define DEFAULT_MWMR_BROADCAST_WAVEGUIDES           3
...
#define DEFAULT_BROADCAST_RATE                      1
#define DEFAULT_PROBABILITY_OF_RETRANSMISSION      .01
#define DEFAULT_TRAFFIC_DISTRIBUTION    TRAFFIC_RANDOM_BROADCAST
...
struct TGlobalParams
{
  ...
  static int mwmr_broadcast_waveguides;
...
struct TFlit
{
  int                src_id;
  int                dst_id;
  vector<int>    	 dst_brd_id;
  int                nearest_gateway_id; // The id of the nearest gateway router
  bool               bc;                 // For the inter PRI communication
  ...
  inline bool operator == (const TFlit& flit) const
  {
    ...
    && flit.bc==bc && flit.flit_type==flit_type && flit.sequence_no==sequence_no
