...
void TProcessingElement::rxProcess() // receiving process
{
  ...
  else
  {
    if(req_rx.read()==1-current_level_rx)
    {
      ...
      //stats.receivedFlit((sc_time_stamp().to_double()+2)/1000, flit_tmp);
      if(TGlobalParams::verbose_mode > VERBOSE_OFF)
      {
        cout << sc_simulation_time() << ": ProcessingElement[" << local_id << "] RECEIVING " << flit_tmp.dst_id << endl;
      }
  ...
  else
  {
    ...
    if(ack_tx.read() == current_level_tx)
    {
      if(!packet_queue.empty())
      {
        ...
        if(TGlobalParams::verbose_mode > VERBOSE_OFF && flit.src_id ==1 )
        {
          cout << sc_time_stamp().to_double()/1000 << ": ProcessingElement[" << local_id << "] SENDING " << flit << " packet type : "<< flit.bc <<endl;
        }
	flit_tx->write(flit);                     // Send the generated flit
	if(TGlobalParams::verbose_mode > VERBOSE_OFF)
	   {
         cout<< sc_time_stamp().to_double()/1000  << "Source_Id --> " <<  flit.src_id <<" Destination_Id --> " <<  flit.dst_id << " packet type : "<< flit.bc << endl; 	   
	   }
...
TFlit TProcessingElement::nextFlit()
{
  ...
  flit.dst_brd_id  = packet.dst_brd_id;
  ...
  if (TGlobalParams::traffic_distribution != TRAFFIC_TABLE_BASED)
    {
		...
		float PIR = atof(PIR_STR.c_str());
		//cout<< "running "<<PIR <<endl;
      ...
      if (shot)
	{
	  switch(TGlobalParams::traffic_distribution)
	    ...
	    case TRAFFIC_HOTSPOT:
	      packet = hotspot();
	      break;
	    case TRAFFIC_RANDOM_BROADCAST:
	      packet = trafficRandom_broadcast();
	      break;
...
//---------------------------------------------------------------------------
TPacket TProcessingElement::trafficRandom_broadcast()
{
  TPacket p;
  p.src_id = local_id;
  ...
  do
  {
    if(broadcast == 0)
    {
		...
		broadcast_count++;
		if(TGlobalParams::verbose_mode > VERBOSE_OFF && p.src_id ==1 )
        {
			cout << " broad cast count1 :" << broadcast_count << " " << 10-TGlobalParams::broadcast_rate<< endl;
		}

		
		if(TGlobalParams::broadcast_rate != 0)
		{
		if(broadcast_count > (10-TGlobalParams::broadcast_rate-1))
		{
			broadcast = 1;
		}
	    }
	...
	{
		p.bc = broadcast;
		for(int i=0; i < randInt(2, 9); i++ )
		{
			p.dst_id = -2;
			int brd_dst = randInt(0, max_id);
			if(brd_dst != local_id)
			{
				p.dst_brd_id.push_back(brd_dst);
			}
		}
		broadcast_count++;
		if(TGlobalParams::verbose_mode > VERBOSE_OFF && p.src_id ==1 )
        {
			cout << " broad cast count2 :" << broadcast_count << " " << 10-TGlobalParams::broadcast_rate<< "destination ids : " << p.dst_brd_id.size()<<endl;
			for (int i=0; i<p.dst_brd_id.size();i++){
        cout << p.dst_brd_id[i] << endl;
    }
		}
