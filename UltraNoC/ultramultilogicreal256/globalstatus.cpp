...
double TGlobalStats::getAverageThroughput()
{
  ...
  for (int x=0; x<TGlobalParams::mesh_dim_y; x++)
    for (int y=0; y<TGlobalParams::mesh_dim_x; y++)
      {
	unsigned int ncomms = noc->git[x][y]->gwi->stats.getTotalCommunications(); 
    
	if (ncomms)
	  {
		  //cout<<"["<<x<<"] "<<"[" << y <<"] "<< "local id: "<< noc->git[x][y]->gwi->local_id_new <<" Total Communication to this tile :"<< ncomms <<endl;
