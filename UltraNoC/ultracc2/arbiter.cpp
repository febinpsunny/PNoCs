			  ...
			  for(int j=0; j<TGlobalParams::mesh_dim_y; j++)
			  {
				  ...
				  src_coord.y = newid2Coord(received_flit.src_id).y;
				    if(TGlobalParams::verbose_mode > VERBOSE_OFF)
				    {
						...
						{
							if(received_flit.bc == 0)
							{
								node_packet_status[src_coord.x][src_coord.y][0] = 1;
								node_packet_status[src_coord.x][src_coord.y][1] = 0;
								
							}
							else
							{
								node_packet_status[src_coord.x][src_coord.y][0] = 0;
								node_packet_status[src_coord.x][src_coord.y][1] = 1;
						...
						if(TGlobalParams::verbose_mode > VERBOSE_OFF)
						{
							cout<< sc_time_stamp().to_double()/1000  << "cooridate [" <<  i <<"] [" << j <<"]"<<"Head flit Presence ->"<< node_packet_status[i][j][0] << endl;
							}
...
void TGlobalarbiter::proc_arbiter_index_tx()
{
  ...
  else
  {
	  if(!reset.read())
	  {
		  for(int w=0; w<(TGlobalParams::mwmr_waveguides - TGlobalParams::mwmr_broadcast_waveguides); w++)
		  {
		  for(int i=0; i<TGlobalParams::mesh_dim_x; i++)
		  {
			  for(int j=0; j<TGlobalParams::mesh_dim_y; j = j+8)
			  {
				   ...
				   for(int k=0; k<8; k++)
				   {
					...
					if(!node_index.empty() && node_index.front() != -1)
					{
						...
						TCoord winner_coord;              // Coverting winner id to co-ordinates
						...
						if(k1 == 1)                     // if channel is available for cluster were the node belongs, then grant permission for that node to send resevation signal and data
						{
						   ...
						   gwi_current_level_tx_ch[winner_coord.x][winner_coord.y] = 1-gwi_current_level_tx_ch[winner_coord.x][winner_coord.y];
							...
							node_packet_status[winner_coord.x][winner_coord.y][0] = 0;
							if(TGlobalParams::verbose_mode > VERBOSE_OFF)
							{
	  ...
	  for(int mbc=0; mbc<TGlobalParams::mwmr_broadcast_waveguides; mbc++)
	  {
	  for(int i=0; i<TGlobalParams::mesh_dim_x; i++)
		  {
			  for(int j=0; j<TGlobalParams::mesh_dim_y; j = j+8)
			  {
				   ...
				   for(int k=0; k<8; k++)
				   {
					...
					if(!node_index.empty() && node_index.front() != -1)
					{
						int winner = node_index.front();
						node_index.clear();
						bool k1 = cb[TGlobalParams::mwmr_waveguides -mbc- 1 ]->assign(winner); // sending indexes of winner nodes to circular queue of current waveguide
						TCoord winner_coord;              // Coverting winner id to co-ordinates
						winner_coord.x = newid2Coord(node_index[0]).x;
						winner_coord.y = newid2Coord(node_index[0]).y;
						if(k1 == 1)                     // if channel is available for cluster were the node belongs, then grant permission for that node to send resevation signal and data
						{
						   ...
						   gwi_current_level_tx_ch[winner_coord.x][winner_coord.y] = 1-gwi_current_level_tx_ch[winner_coord.x][winner_coord.y];
							req_rx_arbiter_ch[winner_coord.x][winner_coord.y].write(gwi_current_level_tx_ch[winner_coord.x][winner_coord.y]); 
							mwmr_channel_no[winner_coord.x][winner_coord.y].write(TGlobalParams::mwmr_waveguides -mbc- 1); // Forwarding channel number to GWI to write data
							node_packet_status[winner_coord.x][winner_coord.y][1] = 0;
							if(TGlobalParams::verbose_mode > VERBOSE_OFF)
							{
								cout<< sc_time_stamp().to_double()/1000  << "cooridateup [" <<  winner_coord.x <<"] [" << winner_coord.y <<"] "<<"index:"<< winner << " channel number for broadcast: "<< TGlobalParams::mwmr_waveguides -mbc- 1 << " channel availability: "<< k1 << endl;
							}
							}
						}
  ...
  else
  {
	  ...
	  for(int i=0; i<4; i++)
	  {
		  ...
		  for(int w=0; w<(TGlobalParams::mwmr_waveguides); w++)
		 {
			...
			//cout<< sc_time_stamp().to_double()/1000<< " cluster No: "<<i<<" Waveguide No: "<<w<<" average Count "<<current_count[w][i]<<endl;
		 }
		 average_count_cluster[i] = count_tmp/float(TGlobalParams::mwmr_waveguides);
		 //cout << sc_time_stamp().to_double()/1000 << " cluster No: "<<i<<" average Count "<<average_count_cluster[i]<<endl;
	  }
	  ...
	  for(int i=0; i<4; i++)
	  {
		 ...
		 //cout<< "cluster No: "<<i<<"Excess Tokens: "<<(int)effective_excess_tokens[i]<<endl;
	  }
	  for(int i=0; i<4; i++)
	  {
		  if(effective_excess_tokens[i] > 1 || effective_excess_tokens[i] < -1)
		  {
			  ...
			  //cout<< sc_time_stamp().to_double()/1000<< "cluster No: "<<i<< " future priorirty1 " << future_priority[i] << " current priority " << current_priority[i] << endl; 
		  }
		  else
		  {
			  future_priority[i] = current_priority[i];
			  //cout<< sc_time_stamp().to_double()/1000<< "cluster No: "<<i<< " future priorirty2 " << future_priority[i] << " current priority " << current_priority[i] << endl; 
		  }
	  }
	  int null_priority_clusters = 0;
	  for(int i=0; i<4; i++)
	  {
		  if(future_priority[i] == 0)
		  {
			  null_priority_clusters++;
		  }
	  }
	  if (null_priority_clusters == 4)
	  {
		  future_priority[0] = 1;
	  }
	  ...
	  // This part of the code sends current priority of clusters, such that token priority of clusters is cahnged
