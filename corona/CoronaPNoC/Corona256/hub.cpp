    ...
    for (int i = 0; i < DIRECTIONS+1; i++)
    {
      ...
      && !buffer[i].IsFull())
      {
		...
		if (TGlobalParams::verbose_mode > VERBOSE_OFF)
        {
          ...
          cout << sc_time_stamp().to_double()/1000 << ": HUBID[" << local_id << "], Input[" << i << "], Received flit: " << received_flit <<" pendind flits "<<buffer[i].
              Size()<< endl;
        }
...
{
  int Port_id_inter = route_data.dst_id - (4 * local_id);
  int dir_out;
