  ...
  else
    {
		  for(int i=0; i< TGlobalParams::mwmr_waveguides; i++)
		  {
			  ...
			  if(previous_transfer_photonic[i] != received_flit_temp.timestamp && !buffer_receive.IsFull() )
			  {
					  ...
					  {
						  cout<< sc_time_stamp().to_double()/1000 << " gwi Source_Id --> " <<  received_flit_temp.src_id <<" gwi Destination_Id --> " <<  received_flit_temp.dst_id <<" Channel number : " << i << endl;	   
					  }
					  ...
					  if(slot_gap==0)
					  {
						  optical_delay = 0.2+1.6;
					  }
					  if(slot_gap>0)
					  {
						  optical_delay = slot_gap*0.2+8;
					  }
					  else
					  {
						  optical_delay = (8+slot_gap)*0.2+8;
					  }
  ...
  else if(!reset.read() ) 
  {
		  if(!buffer_local.IsEmpty() && req_rx_arbiter_ch.read()==1-arbiter_current_level_rx_ch)
		  {
			  ...
			  if(tx_flit.flit_type == FLIT_TYPE_HEAD) 
			  {
				  ...
				  if(TGlobalParams::verbose_mode > VERBOSE_OFF )
				  {
					  cout<< sc_time_stamp().to_double()/1000  << "Source_Id --> " <<  tx_flit.src_id <<" Destination_Id --> " <<  tx_flit.dst_id << endl;
				  }
